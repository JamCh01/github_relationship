...
def mariadb_select(user_name, level, referer, type):
    with mariadb_connection() as cursor:
        SQL = '''SELECT * FROM relationship WHERE user_name=%s AND referer=%s AND level=%s AND type=%s'''
        cursor.execute(SQL, (user_name, referer, level + 1, type))
